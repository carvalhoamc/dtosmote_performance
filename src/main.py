...
def multiclass():
	...
	for r in releases:
		...
		analisys.run_rank_choose_parameters(input_dir + 'average_results_multiclass_' + r + '.csv', kind='multiclass',
		                                    release=r)
...
def biclass():
	...
	for r in releases:
		...
		analisys.run_rank_choose_parameters(input_dir + 'average_results_biclass_' + r + '.csv', kind='biclass',
		                                    release=r)
		analisys.grafico_variacao_alpha(kind='biclass', release=r)
	
	analisys.best_alpha(kind='biclass')
	r = 'v1'
	analisys.run_global_rank(input_dir + 'average_results_biclass_' + r + '.csv', kind='multiclass', release=r)
